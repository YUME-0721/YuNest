...
- 💾 **云端同步与持久化支撑**: 
  ...
  - **集成 GitHub API**：支持一键将数据推送到私有或公开 GitHub 仓库的 `data` 分支（推荐），实现数据更新与代码部署的分离。
- 🚀 **性能与体验优化**: 
...
5. **配置云端同步 (推荐)**: 
   ...
   - **填写 GitHub 仓库**:
     ...
     3. **💡 部署小技巧**: 将数据放在 `data` 分支是为了避免每次保存书签时，GitHub 推送触发 Cloudflare Pages 等平台的代码自动重新部署，从而提升访问性能。
   - **🛡️ 隐私存储方案 (强力推荐)**:
...
6. **数据同步机制 (Data Sync Logic)**:
   - **初次拉取 (Auto Pull)**: 在第一次进入网站（本地无数据）时，如果已设置 GitHub Token 和仓库的环境变量，YuNest 会自动为您从 GitHub 拉取数据，实现开箱即用的跨端体验。
   - **本地优先 (Local First)**: 一旦您在本地开始管理书签或自定义设置，手动刷新将不会覆盖您的本地修改，以防止未同步的变动丢失。
   - **手动同步**: 
     - 点击后台“备份与还原”页下的 **“推送到仓库 (Push)”** 即可更新云端数据。
     - 点击 **“从云侧拉取 (Pull)”** 即可在需要时从云端覆盖本地。
